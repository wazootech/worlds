...
1.  **Segmentation**: Unstructured text is decomposed into semantically coherent
    chunks.
2.  **Fact Extraction (Construct)**: An LLM-based extraction layer identifies
    items and properties, converting narrative flow into formalized facts
    (RDF triples).
3.  **Relational Mapping (Align)**: Extracted items are mapped to a strict
...
- **`triples` table**: Stores the structural data of facts (Subject, Predicate,
  Object) as an append-only chronological ledger.
...
| **Fact**           | A unit of data bound to time, structurally stored as a Triple (Subject, Predicate, Object).|
