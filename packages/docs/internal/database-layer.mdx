...
- **World databases** (one per world):
  ...
  - Created by `initializeDatabase(client)` in `lib/database/init.ts`.
  - When a new world DB is created, the system initializes the schema.

## DatabaseManager interface

Defined in `packages/server/lib/database/manager.ts`:

- `create(id: string): Promise<ManagedDatabase>` — Create a new world DB and
  initialize its schema.
- `get(id: string): Promise<ManagedDatabase>` — Retrieve a `ManagedDatabase` for
  the world.
...
- **TursoDatabaseManager**: Uses Turso API to manage remote databases and
  tokens.
...
- `core/init.ts` imports table and index names from generated files to run DDL
  in order.
