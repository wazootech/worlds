...
This section describes the structure of the application for consistent
implementation and extension. It is the source of truth for architecture,
...
- **Entrypoints:** `packages/server/main.ts` (server), `packages/cli/main.ts`
  (CLI).
- **Import alias:** `#/` maps to `./` within each package; use `#/lib/...`,
  `#/routes/...` for internal imports.
