...
This document describes the application structure to ensure consistent implementation and extension. It serves as the source of truth for architecture, layering, and conventions.
...
- **Entrypoints**: `packages/server/main.ts` (server) and `packages/cli/main.ts` (CLI).
- **Import alias**: The `#/` alias maps to `./` within each package. Use `#/lib/...` or `#/routes/...` for internal imports.
