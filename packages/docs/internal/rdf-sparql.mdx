...
- **`lib/blob/n3.ts`**: Provides utilities to convert between RDF blobs (e.g., N-Quads strings) and in-memory N3 stores.
- **`lib/blob/sparql.ts`**: The `sparql(blob, query, handler)` function executes SPARQL queries using the Comunica engine over an N3 store.

## RDF patching

Centralized in `lib/rdf-patch/mod.ts`, the `handlePatch(client, embeddings, patches)` function processes mutations for the world database:
...
The SPARQL route handler executes queries through the following pipeline:
