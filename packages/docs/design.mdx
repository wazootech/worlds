...
description: "Structure, architecture, layering, and conventions of the Worlds API."
---
...
- **admin?**: `{ apiKey: string }` — Required for production; used by authorizeRequest to allow access to all v1 routes.
...
- **embeddings/embeddings.ts:** Interface **Embeddings** — `embed(text: string): Promise<number[]>`, **dimensions: number**.
