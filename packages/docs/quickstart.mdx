...
quickstart skips the theory and drops you straight into the SDK to execute your
...
engine, inject a fact, and retrieve it deterministically.
...
<Steps>
  ...
  <Step title="Observe the execution">
    ...
    console.log("Memory retrieved:", JSON.stringify(result, null, 2));
    ```
  ...
  <Step title="Analyze the output">
    ...
    Memory retrieved:
    {
      ...
      "results": {
        "bindings": [
          {
            "subject": { "type": "uri", "value": "system" },
            "predicate": { "type": "uri", "value": "status" },
            "object": { "type": "literal", "value": "operational" }
          }
        ]
      }
    }
...
Now that you have securely connected to your cloud instance, integrate the SDK
