...
- Item-to-value: Connects an item to a raw data value, adding searchable detail
  but acting as a terminal point e.g., `user:ethan` -> `schema:givenName` ->
  `"Ethan"`
...
```turtle N-Triples
<http://corp.example/acme> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <https://schema.org/Organization> .
```
...
that Acme Corp is an organization with the legal name "Acme Corp":
...
```turtle N-Triples
<http://corp.example/acme> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <https://schema.org/Organization> .
<http://corp.example/acme> <https://schema.org/legalName> "Acme Corp" .
```
