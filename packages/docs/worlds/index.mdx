...
An item can be identified by a unique
[IRI](https://en.wikipedia.org/wiki/Internationalized_Resource_Identifier) and
can be classified by types.

For example, the item of IRI `user:ethan` is of the type `schema:Person`.
...
Items can be formalized graphically as nodes or mathematically mapped using
standard RDF serialization like Turtle. The type of every item is also
represented as a fact—it's facts all the way down.
