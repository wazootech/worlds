...
<ResponseField name="Subject" type="Term">
  The item you are describing e.g., `user:me`
</ResponseField>
...
- Item-to-item: Connects two distinct items e.g., `user:me` ->
  `schema:worksFor` -> `wazoo:wazoo`
- Item-to-value: Connects an item to a raw data value, adding searchable detail
  but acting as a terminal point e.g., `user:me` -> `schema:givenName` ->
  `"Ethan"`
...
graph LR
    ...
    A((user:me)) -->|schema:worksFor| B((wazoo:wazoo))
...
```turtle N-Triples
<https://etok.me/me> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <https://schema.org/Person> .
```
...
```turtle N-Triples
<https://etok.me/me> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <https://schema.org/Person> .
<https://etok.me/me> <https://schema.org/givenName> "Ethan" .
```
...
ASK WHERE {
  <https://etok.me/me> wazoo:worksFor <https://wazoo.dev/wazoo> .
}
```
