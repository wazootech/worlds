title: "Whitepaper" description: "Malleable Knowledge at the Edge for
...
**Worlds** is a managed neuro-symbolic infrastructure layer—a "World
...
persistence, Worlds enables agents to maintain mutable, structured knowledge
...
**Fluid Intelligence** and **Outcome-Based Determinism**. Worlds provides the
...
intelligence is defined not by the volume of retained data, but by the
efficiency with which a system adapts to novel, untrained environments. The
...
We propose **Worlds**, a system designated to provide **Malleable Knowledge**
...
- **The Local-First Manifesto**: A commitment to high-privacy, durable, and
...
- **The "Small Web"**: A future where users maintain autonomy, control their
  data, and operate in an interoperable ecosystem built on permissive
  open-source standards (Apache 2.0) rather than proprietary locks.
...
The following SQL snippet demonstrates this logic implemented within our SQLite
engine:
...
Higher-latency cross-encoder models can be used to rerank the top-K results from
the hybrid search, providing a more nuanced semantic alignment before the data
reaches the agent's context.

### Human-In-The-Loop (HITL)

When the system identifies low-confidence matches or multiple conflicting
entities, the "Malleable" nature of Worlds allows the UI to present
disambiguation prompts to the user.

### Outcome-Based Determinism

By utilizing reification in Context Graphs, relationships become first-class
entities. If a structural anomaly is detected during traversal, the system
triggers an intervention. This shifts the focus of trust from eliminating
uncertainty to managing it through rigorous, auditable verification.
...
Worlds is designed to be **Agent-Ready** from the first request. We embrace the
**Model Context Protocol (MCP)** as an interoperable standard. By operating as a
dedicated Context Layer, Worlds allows host applications—such as contextual
coding agents—to securely interface with private knowledge graphs and
autonomously index raw SDK source code without hallucinations.
...
enabling developers to bootstrap memory-augmented agents with resilient
cross-session memory in seconds.
